...
def text_center(target: np.ndarray, score=0.5, random_use=True, text_filter=None):
    text_list = text_locations(target, score)
    if text_list is not None:
        if text_filter is not None:
            if type(text_filter) == str:
                text_list = [c for c in text_locations(target, score) if text_filter in c[-1][0]]
            else:
                text_list = [c for c in text_locations(target, score) if c[-1][0] in text_filter]
        text_list = [[(int(sum([row[0] for row in c[0]]) / 4.0 + 5 * random.random()),
                       int(sum([row[1] for row in c[0]]) / 4.0 + 5 * random.random())), c[-1][0], c[-1][1]] for c in
                     text_list]
    return text_list
